...
import { listZipEntryNames, openZipEntryStream } from "../../core/zip.js";
import { parseGtfsTimeToSeconds } from "../../utils/gtfsTime.js";
import type { QueenslandBusDatasetConfig } from "./config.js";
import { getQueenslandBusRowCounts } from "./rowCounts.js";
...
async function importStops(db: SQLiteDatabase, gtfsZipPath: string): Promise<number> {
  const stream = await openZipEntryStream(gtfsZipPath, "stops.txt");
...
async function importRoutes(db: SQLiteDatabase, gtfsZipPath: string): Promise<number> {
  const stream = await openZipEntryStream(gtfsZipPath, "routes.txt");
  const statement = db.prepare(`
    INSERT INTO routes (
      route_id,
      route_short_name,
...
async function importTrips(db: SQLiteDatabase, gtfsZipPath: string): Promise<number> {
  const stream = await openZipEntryStream(gtfsZipPath, "trips.txt");
  const statement = db.prepare(`
    INSERT INTO trips (
      trip_id,
      route_id,
...
async function importStopTimes(db: SQLiteDatabase, gtfsZipPath: string): Promise<number> {
  const stream = await openZipEntryStream(gtfsZipPath, "stop_times.txt");
  const statement = db.prepare(`
...
async function importCalendar(db: SQLiteDatabase, gtfsZipPath: string, entryName: string): Promise<number> {
  const stream = await openZipEntryStream(gtfsZipPath, entryName);
  const statement = db.prepare(`
...
async function importCalendarDates(db: SQLiteDatabase, gtfsZipPath: string, entryName: string): Promise<number> {
  const stream = await openZipEntryStream(gtfsZipPath, entryName);
  const statement = db.prepare(`
...
export async function buildQueenslandBus(dataset: QueenslandBusDatasetConfig): Promise<void> {
  const outputDirectory = dirname(dataset.output.sqlitePath);
  const manifestDirectory = dirname(dataset.output.manifestPath);
  const gtfsZipPath = resolve(dataset.tempDirectory, "SEQ_GTFS.zip");
  ...
  logStep(`Downloading GTFS archive from ${dataset.sourceUrl}`);
  await downloadFile(dataset.sourceUrl, gtfsZipPath);

  const entryNames = new Set(await listZipEntryNames(gtfsZipPath));
  for (const requiredFile of dataset.requiredFiles) {
    if (!entryNames.has(requiredFile)) {
      throw new Error(`Missing required GTFS file ${requiredFile} in ${gtfsZipPath}`);
    }
  }
  ...
  try {
    ...
    const importedCounts: Record<string, number> = {
      stops: await importStops(db, gtfsZipPath),
      routes: await importRoutes(db, gtfsZipPath),
      trips: await importTrips(db, gtfsZipPath),
      stop_times: await importStopTimes(db, gtfsZipPath),
      calendar: await importOptional(entryNames, "calendar.txt", () => importCalendar(db, gtfsZipPath, "calendar.txt")),
      calendar_dates: await importOptional(
        entryNames,
        "calendar_dates.txt",
        () => importCalendarDates(db, gtfsZipPath, "calendar_dates.txt"),
      ),
    };
    ...
    const rowCounts = getQueenslandBusRowCounts(db);
