...
import type { VictoriaTrainDatasetConfig } from "./config.js";
...
function validateTrainStopTagging(db: SQLiteDatabase, dataset: VictoriaTrainDatasetConfig): void {
  const routeTypesSql = dataset.trainRouteTypes.join(", ");
  const matchingRoutes = db
    .prepare(`SELECT COUNT(*) AS count FROM routes WHERE route_type IN (${routeTypesSql})`)
    .get() as { count: number };
  ...
  const taggedStops = db
    .prepare("SELECT COUNT(*) AS count FROM stops WHERE route_type = ?")
    .get(dataset.stopRouteType) as { count: number };

  assert(
    taggedStops.count > 0,
    `Expected at least one train stop with route_type = ${dataset.stopRouteType}`,
  );
}

export async function validateVictoriaTrain(dataset: VictoriaTrainDatasetConfig): Promise<void> {
  ...
  try {
    ...
    validateTrainStopTagging(db, dataset);
