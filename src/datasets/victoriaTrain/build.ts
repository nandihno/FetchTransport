...
import type { VictoriaTrainDatasetConfig, VictoriaTrainFeed } from "./config.js";
import { getVictoriaTrainRowCounts } from "./rowCounts.js";
import { CREATE_SCHEMA_SQL, createTagTrainStopsSql } from "./schema.js";
...
type ExtractedFeed = VictoriaTrainFeed & {
...
function namespacedValue(feed: VictoriaTrainFeed, value: string): string {
  return `${feed.modeFolder}:${value}`;
}
...
async function importStops(db: SQLiteDatabase, feed: ExtractedFeed): Promise<number> {
  const stream = await openZipEntryStream(feed.extractedZipPath, "stops.txt");
...
async function importRoutes(db: SQLiteDatabase, feed: ExtractedFeed): Promise<number> {
  const stream = await openZipEntryStream(feed.extractedZipPath, "routes.txt");
  const statement = db.prepare(`
  ...
  for await (const record of streamCsvRecords(stream)) {
    inserter.push([
      namespacedValue(feed, requiredValue(record, "route_id")),
...
async function importTrips(db: SQLiteDatabase, feed: ExtractedFeed): Promise<number> {
  const stream = await openZipEntryStream(feed.extractedZipPath, "trips.txt");
  const statement = db.prepare(`
  ...
  for await (const record of streamCsvRecords(stream)) {
    inserter.push([
      namespacedValue(feed, requiredValue(record, "trip_id")),
      namespacedValue(feed, requiredValue(record, "route_id")),
      namespacedValue(feed, requiredValue(record, "service_id")),
...
async function importStopTimes(db: SQLiteDatabase, feed: ExtractedFeed): Promise<number> {
  const stream = await openZipEntryStream(feed.extractedZipPath, "stop_times.txt");
  const statement = db.prepare(`
...
async function importCalendar(db: SQLiteDatabase, feed: ExtractedFeed, entryName: string): Promise<number> {
  const stream = await openZipEntryStream(feed.extractedZipPath, entryName);
  const statement = db.prepare(`
...
async function importCalendarDates(db: SQLiteDatabase, feed: ExtractedFeed, entryName: string): Promise<number> {
  const stream = await openZipEntryStream(feed.extractedZipPath, entryName);
  const statement = db.prepare(`
...
async function importOptional(feed: ExtractedFeed, entryName: string, importer: () => Promise<number>): Promise<number> {
  if (!feed.entryNames.has(entryName)) {
    return 0;
  }
...
async function extractFeeds(dataset: VictoriaTrainDatasetConfig, outerZipPath: string): Promise<ExtractedFeed[]> {
  const extractedFeeds: ExtractedFeed[] = [];

  for (const feed of dataset.feeds) {
    const extractedZipPath = resolve(dataset.tempDirectory, `${feed.modeFolder}-${basename(feed.nestedArchivePath)}`);

    logStep(`Extracting nested archive ${feed.nestedArchivePath}`);
    await extractZipEntryToFile(outerZipPath, feed.nestedArchivePath, extractedZipPath);

    const entryNames = new Set(await listZipEntryNames(extractedZipPath));
    for (const requiredFile of dataset.requiredFiles) {
      if (!entryNames.has(requiredFile)) {
        throw new Error(`Missing required GTFS file ${requiredFile} in ${extractedZipPath}`);
      }
    }
  ...
  return extractedFeeds;
}

export async function buildVictoriaTrain(dataset: VictoriaTrainDatasetConfig): Promise<void> {
  ...
  const feeds = await extractFeeds(dataset, outerZipPath);
  ...
  try {
    ...
    for (const feed of feeds) {
      await importStops(db, feed);
      await importRoutes(db, feed);
      await importTrips(db, feed);
      await importStopTimes(db, feed);
      await importOptional(feed, "calendar.txt", () => importCalendar(db, feed, "calendar.txt"));
      await importOptional(feed, "calendar_dates.txt", () => importCalendarDates(db, feed, "calendar_dates.txt"));
    }

    executeSql(db, createTagTrainStopsSql(dataset.stopRouteType, dataset.trainRouteTypes));
    const rowCounts = getVictoriaTrainRowCounts(db);
